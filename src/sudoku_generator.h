...
bool fill_with_valid_solution(
    Grid *grid,
    Coord curr_coord = make_pair(0, 0)
) {
    ...
    auto rnd_seed = chrono::system_clock::now().time_since_epoch().count();
    shuffle(
        values.begin(),
        values.end(),
        default_random_engine(rnd_seed));

    for (auto value : values) {
        grid->update(curr_coord, value);
        if (curr_coord == make_pair(8, 8)) return true;  // Done
